...
export default function Clients() {
    ...
    const setActive = (row) => {
        setAppContext(prev => ({
            ...prev,
            activeClient: row.idEntity,
            activeClientName: row.entityTitle,
            activeBackground: row.backgroundColor,
            activeColor: row.backgroundColor,
            authState: 'home'
        }));
    };
    ...
    const cbUpdate = (error, data) => {
        if (error === null && !data?.error) {
            setAppContext(prev => ({
                ...prev,
                activeClientName: draft.entityTitle,
                activeBackground: draft.backgroundColor,
                activeColor: draft.backgroundColor || '#4b8d96',  // ← Also update activeColor here
            }));
            cancelEdit()
            cdo.list(cbList)
        }
