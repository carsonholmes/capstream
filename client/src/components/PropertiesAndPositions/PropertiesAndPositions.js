...
<br /><br /><br /><br />
      <div className={`PropertiesAndPositions ${appContext?.darkMode ? 'dark-mode' : 'light-mode'}`}>
        {/* Top: Select existing / create new */}
        <Card cardHeading=''>
          <div className='pap-select-grid'>
            <Select
              id='existingRecord'
              dataCol='existingRecord'
              label='Select a Position to Edit'
              type='text'
              value={selectedId ?? -1}
              data={[{ value: -1, caption: 'Select a Property & Position record to edit' }, ...records.map(r => ({ value: String(r.idEntity), caption: renderRecordOptionLabel(r) }))]}
              onChange={(col, v) => setSelectedId(v === -1 ? null : String(v))}
            />
            <Button styleName='primary sign-in home' onClick={clearForm}>Add New</Button>
          </div>
        </Card>

        <Card cardHeading={row.idEntity ? 'Edit Property & Position' : 'Create Property + Position'}>
          {/* Property */}
          <div className='pap-section-title'>Property</div>
          ...
          {/* <div className='pap-row'> */}
            <TextBox id='partnershipLP' dataCol='partnershipLP' value={row.partnershipLP} placeholder='Partnership (LP)' onChange={update} onSubmit={onSave} />
            <div className='pap-date'>
              <DatePicker id='acquisitionDate' dataCol='acquisitionDate' placeholder='Acquisition Date' value={row.acquisitionDate || null} onChange={update} />
            </div>
          {/* </div> */}

          <div className='pap-row'>
            <div className='pap-span-2'>
              <TextArea id='propertyNotes' dataCol='propertyNotes' value={row.propertyNotes} placeholder='Notes' onChange={update} />
            </div>
          </div>

          {/* Position */}
          <div className='pap-section-title'>Your Position</div>
          <div className='pap-row'>
            <TextBox id='lpEntity' dataCol='lpEntity' value={row.lpEntity} placeholder='LP Entity' onChange={update} onSubmit={onSave} />
            <TextBox
              id='investedEquity'
              dataCol='investedEquity'
              value={row.investedEquity ?? ''}
              placeholder='Invested equity'
              onChange={update}
              onSubmit={onSave}
              type='number'
              step='0.01'
          ...
          <div className='pap-row'>
            ...
            <div>
              <TextBox
                ...
                regex={REGEX_PERCENT}
              />
              <div className='pap-help'>Leave blank if unknown or single-investor</div>
            </div>
          </div>
          <div className='pap-row'>
            <TextBox
              id='prefReturnPct'
              dataCol='prefReturnPct'
              value={row.prefReturnPct ?? ''}
          ...
          {/* Targets */}
          <div className='pap-section-title'>Targets from OM</div>
          <div className='pap-row cols-3'>
            <TextBox
              id='targetIRR'
              dataCol='targetIRR'
              value={row.targetIRR ?? ''}
            ...
            <TextBox
              ...
              dataCol='targetEquityMultiple'
              value={row.targetEquityMultiple ?? ''}
              placeholder='Target Equity Multiple'
              onChange={update}
              onSubmit={onSave}
              type='number'
              step='0.01'
        ...
        {/* Attachments */}
        <Card cardHeading='Attachments'>
          <div className='pap-attachments'>
            <div className='pap-drop'>Drop OM PDF here (optional)</div>
            <input className='pap-file' type='file' onChange={onFilePick} />
            {row.attachments?.length > 0 && (
              <ul className='pap-files'>
      ...
      {/* <div className='pap-wrapper'>
            ...
            <TextBox id='partnershipLP' dataCol='partnershipLP' value={row.partnershipLP} placeholder='Partnership (LP)' onChange={update} onSubmit={onSave} />
            <DatePicker id='acquisitionDate' dataCol='acquisitionDate' placeholder='Acquisition Date' value={row.acquisitionDate || null} onChange={update} />
            <TextArea style={{gridColumn: 'span 2', justifySelf: 'center'}} id='propertyNotes' dataCol='propertyNotes' value={row.propertyNotes} placeholder='Notes' onChange={update} />
      </div> */}
