...
CREATE TABLE IF NOT EXISTS `factory`.`modify-code` (
  ...
  `dtCreated` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `changeSummary` VARCHAR(511) NULL DEFAULT NULL,
...
DELIMITER $$
USE `factory`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertModifyCode`(IN p_idApp CHAR(36), IN p_changeSummary VARCHAR(511), IN p_dbChanges TEXT, IN p_newClientImports TEXT, IN p_newServerImports TEXT, IN p_version INT, IN p_fixErrors TINYINT, IN p_errorText TEXT)
BEGIN
...
BEGIN SELECT BIN_TO_UUID(idApp) as idApp, dtCreated, changeSummary, dbChanges, newClientImports, newServerImports, version FROM `modify-code` WHERE idModifyCode = p_idModifyCode; END$$
